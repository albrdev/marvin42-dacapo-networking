...
bool SendCommand(TCPClient& client, const std::string& command, const std::vector<std::string>& arguments, const long timeout)
{
    if(command == "run")
    {
...
int main(int argc, char *argv[])
{
    ...
    while(true)
    {
        ...
        TCPClient client(addresses[0].c_str(), options.GetPort(), options.GetTimeout() < 0 ? 0 : options.GetTimeout());
